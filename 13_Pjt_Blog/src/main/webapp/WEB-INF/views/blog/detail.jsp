...
<jsp:include page="../layout/header.jsp">
  <jsp:param name="title" value="${blog.blogNo}블로그" />
</jsp:include>
  		  	...
  		  	$.each(resData.commentList, function(i, comment){
  		  	  ...
  		  	  if(comment.state == -1){
  		  	      ...
  		  	      str += comment.memberDTO.name;
  		  	  	  str += ' - ' + comment.content;
	  		  	  if('${sessionScope.loginId}' != ''){
	  		  		  if('${sessionScope.loginId}' == comment.memberDTO.id && comment.status == 1){
			...
			<c:forEach items="${blogList}" var="blog" varStatus="vs">
				<tr>
					...
					<td>
						...
						<c:if test="${sessionScope.loginId eq blog.memberDTO.id}">
							<a href="${contextPath}/blog/detail.do?blogNo=${blog.blogNo}">${blog.title}</a>   <!-- 조회수 안 늘음 -->
						</c:if>
