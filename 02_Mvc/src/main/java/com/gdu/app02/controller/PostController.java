...
public class PostController {
	...
	public String detail(HttpServletRequest request) throws Exception {  // name, age 파라미터가 있다.
		...
		return "redirect:/post/list.do?name=" + URLEncoder.encode(name, "UTF-8") + "&age=" + age;   // redirect의 return 경로는 mapping이여야 한다.
		// ㄴ /post/list.do 매핑으로 이동하시오!      // ㄴ name, age 파라미터를 다시 붙인다!
		
		
	}
	
	@GetMapping("/post/list.do")  
	public String list(HttpServletRequest request,  // request 안에 name, age 파라미터가 있다.
						Model model) {  
